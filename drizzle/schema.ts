import { pgTable, index, foreignKey, pgPolicy, check, uuid, timestamp, text, date, numeric, unique, varchar, jsonb, integer, boolean } from "drizzle-orm/pg-core"
...
export const invoices = pgTable("invoices", {
	...
	taxAmount: numeric("tax_amount", { precision: 10, scale:  2 }).generatedAlwaysAs(sql`((subtotal * tax_rate) / (100)::numeric)`),
	total: numeric({ precision: 10, scale:  2 }).generatedAlwaysAs(sql`(subtotal + ((subtotal * tax_rate) / (100)::numeric))`),
	notes: text(),
	...
	pgPolicy("Users can delete their own invoices", { as: "permissive", for: "delete", to: ["public"], using: sql`(auth.uid() = user_id)` }),
	pgPolicy("Users can insert their own invoices", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can read their own invoices", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("Users can update their own invoices", { as: "permissive", for: "update", to: ["public"] }),
...
export const profiles = pgTable("profiles", {
	...
	pgPolicy("Admin full access", { as: "permissive", for: "all", to: ["service_role"], using: sql`true` }),
	pgPolicy("Public profiles are viewable", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("Users can create their own profile", { as: "permissive", for: "insert", to: ["public"] }),
...
export const invoiceItems = pgTable("invoice_items", {
	...
	pgPolicy("Users can delete invoice items through invoices", { as: "permissive", for: "delete", to: ["public"], using: sql`(EXISTS ( SELECT 1
   FROM invoices
  WHERE ((invoices.id = invoice_items.invoice_id) AND (invoices.user_id = auth.uid()))))` }),
	pgPolicy("Users can insert invoice items through invoices", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can read invoice items through invoices", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("Users can update invoice items through invoices", { as: "permissive", for: "update", to: ["public"] }),
]);
...
export const clients = pgTable("clients", {
	...
	pgPolicy("Users can delete their own clients", { as: "permissive", for: "delete", to: ["public"], using: sql`(auth.uid() = user_id)` }),
	pgPolicy("Users can insert their own clients", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can read their own clients", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("Users can update their own clients", { as: "permissive", for: "update", to: ["public"] }),
]);
