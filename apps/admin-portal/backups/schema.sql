...
--
-- Name: enterprise_dashboard_stats; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.enterprise_dashboard_stats AS
 SELECT e.id AS enterprise_id,
    count(DISTINCT i.id) AS total_invoices,
    COALESCE(sum(i.total), (0)::numeric) AS total_revenue,
    count(DISTINCT
        CASE
            WHEN (lower(i.status) = 'pending'::text) THEN i.id
            ELSE NULL::uuid
   ...
   FROM (((((((((((public.enterprises e
     LEFT JOIN public.invoices i ON ((i.enterprise_id = e.id)))
     LEFT JOIN public.products p ON ((p.enterprise_id = e.id)))
     LEFT JOIN public.employees emp ON ((emp.enterprise_id = e.id)))
     LEFT JOIN public.departments d ON ((d.enterprise_id = e.id)))
     LEFT JOIN public.jobs j ON ((j.enterprise_id = e.id)))
     LEFT JOIN public.clients cl ON ((cl.enterprise_id = e.id)))
     LEFT JOIN public.companies co ON ((co.enterprise_id = e.id)))
     LEFT JOIN public.vendors v ON ((v.enterprise_id = e.id)))
     LEFT JOIN public.offices o ON ((o.enterprise_id = e.id)))
     LEFT JOIN public.warehouses w ON ((w.enterprise_id = e.id)))
     LEFT JOIN public.branches b ON ((b.enterprise_id = e.id)))
  GROUP BY e.id;
