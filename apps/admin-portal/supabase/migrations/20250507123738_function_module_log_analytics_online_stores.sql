...
CREATE OR REPLACE FUNCTION public.module_log_online_stores()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE TRIGGER trigger_online_stores_activity_log    
AFTER INSERT OR UPDATE OR DELETE ON public.online_stores
FOR EACH ROW EXECUTE FUNCTION public.module_log_online_stores();


-- Drop existing function if needed
DROP FUNCTION IF EXISTS get_module_analytics_online_stores;

-- Create new function with full implementation
CREATE OR REPLACE FUNCTION get_module_analytics_online_stores(
...
RETURNS TABLE (
  ...
  online_stores_added BIGINT,
  online_stores_removed BIGINT,
  online_stores_updated BIGINT
) AS $$
DECLARE
...
BEGIN
  ...
  SELECT 
    ts.period_start,
    COUNT(al.*) FILTER (WHERE al.action_type = 'CREATED')::BIGINT AS online_stores_added,
    COUNT(al.*) FILTER (WHERE al.action_type = 'DELETED')::BIGINT AS online_stores_removed,
    COUNT(al.*) FILTER (WHERE al.action_type = 'UPDATED')::BIGINT AS online_stores_updated
  FROM time_series ts
  LEFT JOIN activity_logs al ON 
    date_trunc(time_interval, al.created_at AT TIME ZONE 'UTC') = ts.period_start
    AND al.target_type = 'ONLINE_STORE'
...
EXCEPTION
  WHEN others THEN
    RAISE WARNING 'Error in get_module_analytics_online_stores: %', SQLERRM;
    RETURN;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;

-- Set permissions
REVOKE ALL ON FUNCTION get_module_analytics_online_stores FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_module_analytics_online_stores TO authenticated;

-- Create indexes
CREATE INDEX IF NOT EXISTS activity_logs_online_stores_created_at_idx
ON activity_logs (created_at)
WHERE target_type = 'ONLINE_STORE';

CREATE INDEX IF NOT EXISTS activity_logs_online_stores_action_idx
