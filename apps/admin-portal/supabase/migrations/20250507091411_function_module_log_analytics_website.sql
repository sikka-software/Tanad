...
CREATE OR REPLACE FUNCTION public.module_log_websites()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE TRIGGER trigger_websites_activity_log    
AFTER INSERT OR UPDATE OR DELETE ON public.websites
FOR EACH ROW EXECUTE FUNCTION public.module_log_websites();


-- Drop existing function if needed
DROP FUNCTION IF EXISTS get_module_analytics_websites;

-- Create new function with full implementation
CREATE OR REPLACE FUNCTION get_module_analytics_websites(
...
RETURNS TABLE (
  ...
  websites_added BIGINT,
  websites_removed BIGINT,
  websites_updated BIGINT
) AS $$
DECLARE
...
BEGIN
  ...
  SELECT 
    ts.period_start,
    COUNT(al.*) FILTER (WHERE al.action_type = 'CREATED')::BIGINT AS websites_added,
    COUNT(al.*) FILTER (WHERE al.action_type = 'DELETED')::BIGINT AS websites_removed,
    COUNT(al.*) FILTER (WHERE al.action_type = 'UPDATED')::BIGINT AS websites_updated
  FROM time_series ts
  LEFT JOIN activity_logs al ON 
    date_trunc(time_interval, al.created_at AT TIME ZONE 'UTC') = ts.period_start
    AND al.target_type = 'WEBSITE'
...
EXCEPTION
  WHEN others THEN
    RAISE WARNING 'Error in get_module_analytics_websites: %', SQLERRM;
    RETURN;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;

-- Set permissions
REVOKE ALL ON FUNCTION get_module_analytics_websites FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_module_analytics_websites TO authenticated;

-- Create indexes
CREATE INDEX IF NOT EXISTS activity_logs_websites_created_at_idx
ON activity_logs (created_at)
WHERE target_type = 'WEBSITE';

CREATE INDEX IF NOT EXISTS activity_logs_websites_action_idx
