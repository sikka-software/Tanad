...
CREATE OR REPLACE FUNCTION public.module_log_purchases()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE TRIGGER trigger_purchases_activity_log    
AFTER INSERT OR UPDATE OR DELETE ON public.purchases
FOR EACH ROW EXECUTE FUNCTION public.module_log_purchases();


-- Drop existing function if needed
DROP FUNCTION IF EXISTS get_module_analytics_purchases;

-- Create new function with full implementation
CREATE OR REPLACE FUNCTION get_module_analytics_purchases(
...
RETURNS TABLE (
  ...
  purchases_added BIGINT,
  purchases_removed BIGINT,
  purchases_updated BIGINT
) AS $$
DECLARE
...
BEGIN
  ...
  SELECT 
    ts.period_start,
    COUNT(al.*) FILTER (WHERE al.action_type = 'CREATED')::BIGINT AS purchases_added,
    COUNT(al.*) FILTER (WHERE al.action_type = 'DELETED')::BIGINT AS purchases_removed,
    COUNT(al.*) FILTER (WHERE al.action_type = 'UPDATED')::BIGINT AS purchases_updated
  FROM time_series ts
  LEFT JOIN activity_logs al ON 
    date_trunc(time_interval, al.created_at AT TIME ZONE 'UTC') = ts.period_start
    AND al.target_type = 'PURCHASE'
...
EXCEPTION
  WHEN others THEN
    RAISE WARNING 'Error in get_module_analytics_purchases: %', SQLERRM;
    RETURN;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;

-- Set permissions
REVOKE ALL ON FUNCTION get_module_analytics_purchases FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_module_analytics_purchases TO authenticated;

-- Create indexes
CREATE INDEX IF NOT EXISTS activity_logs_purchases_created_at_idx
ON activity_logs (created_at)
WHERE target_type = 'PURCHASE';

CREATE INDEX IF NOT EXISTS activity_logs_purchases_action_idx
