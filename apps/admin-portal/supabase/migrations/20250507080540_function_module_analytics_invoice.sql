...
DROP FUNCTION IF EXISTS get_module_analytics_invoice;

-- Create new function with full implementation
CREATE OR REPLACE FUNCTION get_module_analytics_invoice(
...
RETURNS TABLE (
  ...
  invoices_added BIGINT,
  invoices_removed BIGINT,
  invoices_updated BIGINT
) AS $$
DECLARE
...
BEGIN
  ...
  SELECT 
    ts.period_start,
    COUNT(al.*) FILTER (WHERE al.action_type = 'CREATED')::BIGINT AS invoices_added,
    COUNT(al.*) FILTER (WHERE al.action_type = 'DELETED')::BIGINT AS invoices_removed,
    COUNT(al.*) FILTER (WHERE al.action_type = 'UPDATED')::BIGINT AS invoices_updated
  FROM time_series ts
  LEFT JOIN activity_logs al ON 
    date_trunc(time_interval, al.created_at AT TIME ZONE 'UTC') = ts.period_start
    AND al.target_type = 'INVOICE'
...
EXCEPTION
  WHEN others THEN
    RAISE WARNING 'Error in get_module_analytics_invoice: %', SQLERRM;
    RETURN;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;

-- Set permissions
REVOKE ALL ON FUNCTION get_module_analytics_invoice FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_module_analytics_invoice TO authenticated;

-- Create indexes
CREATE INDEX IF NOT EXISTS activity_logs_invoice_created_at_idx
ON activity_logs (created_at)
WHERE target_type = 'INVOICE';

CREATE INDEX IF NOT EXISTS activity_logs_invoice_action_idx
