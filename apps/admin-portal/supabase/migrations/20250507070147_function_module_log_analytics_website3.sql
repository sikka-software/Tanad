...
CREATE OR REPLACE FUNCTION public.module_log_website()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE TRIGGER trigger_websites_activity_log
AFTER INSERT OR UPDATE OR DELETE ON public.websites
FOR EACH ROW EXECUTE FUNCTION public.module_log_website();


-- Drop existing function if needed
DROP FUNCTION IF EXISTS get_module_analytics_website;

-- Create new function with full implementation
CREATE OR REPLACE FUNCTION get_module_analytics_website(
...
EXCEPTION
  WHEN others THEN
    RAISE WARNING 'Error in get_module_analytics_website: %', SQLERRM;
    RETURN;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;

-- Set permissions
REVOKE ALL ON FUNCTION get_module_analytics_website FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_module_analytics_website TO authenticated;

-- Create indexes
CREATE INDEX IF NOT EXISTS activity_logs_website_created_at_idx
ON activity_logs (created_at)
WHERE target_type = 'WEBSITE';

CREATE INDEX IF NOT EXISTS activity_logs_website_action_idx
