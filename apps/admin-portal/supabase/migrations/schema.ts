...
export const companies = pgTable("companies", {
	...
	index("companies_is_active_idx").using("btree", table.is_active.asc().nullsLast().op("bool_ops")),
...
export const enterprises = pgTable("enterprises", {
	...
	pgPolicy("Enable delete for superadmins", { as: "permissive", for: "delete", to: ["authenticated"], using: sql`(EXISTS ( SELECT 1
   FROM user_roles
  WHERE ((user_roles.user_id = auth.uid()) AND (user_roles.enterprise_id = enterprises.id) AND (user_roles.role = 'superadmin'::text))))` }),
	pgPolicy("Enable insert for authenticated users", { as: "permissive", for: "insert", to: ["authenticated"] }),
	pgPolicy("Enable read access for users", { as: "permissive", for: "select", to: ["authenticated"] }),
	pgPolicy("Enable update for superadmins", { as: "permissive", for: "update", to: ["authenticated"] }),
]);
...
export const user_roles = pgTable("user_roles", {
	...
	index("user_roles_role_idx").using("btree", table.role.asc().nullsLast().op("text_ops")),
	...
	primaryKey({ columns: [table.user_id, table.role, table.enterprise_id], name: "user_roles_user_id_role_enterprise_id_pk"}),
]);
