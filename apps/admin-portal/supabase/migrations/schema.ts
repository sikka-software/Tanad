import { pgTable, index, foreignKey, uuid, timestamp, text, jsonb, unique, pgSchema, json, varchar, pgPolicy, numeric, uniqueIndex, check, bigserial, boolean, smallint, inet, date, bigint, primaryKey, pgView, pgEnum } from "drizzle-orm/pg-core"
...
export const activity_log_action_type = pgEnum("activity_log_action_type", ['INSERT', 'UPDATE', 'DELETE', 'CREATED', 'UPDATED', 'DELETED'])
export const activity_log_target_type = pgEnum("activity_log_target_type", ['ENTERPRISE', 'USER', 'EMPLOYEE', 'ROLE', 'PERMISSION', 'INVOICE', 'EXPENSE', 'SETTING', 'NOTIFICATION', 'SYSTEM'])
export const activity_target_type = pgEnum("activity_target_type", ['USER', 'ROLE', 'COMPANY', 'CLIENT', 'INVOICE', 'EXPENSE', 'QUOTE', 'BRANCH', 'VENDOR', 'OFFICE', 'WAREHOUSE', 'PURCHASE', 'PRODUCT', 'EMPLOYEE', 'DEPARTMENT', 'SALARY', 'SERVER', 'JOB_LISTING', 'APPLICANT', 'JOB', 'TEMPLATE', 'DOCUMENT', 'ENTERPRISE_SETTINGS', 'EMPLOYEE_REQUEST', 'DOMAIN', 'WEBSITE', 'ONLINE_STORE', 'CAR', 'TRUCK'])
export const app_permission = pgEnum("app_permission", ['users.create', 'users.read', 'users.update', 'users.delete', 'users.export', 'users.invite', 'roles.create', 'roles.read', 'roles.update', 'roles.delete', 'roles.export', 'roles.assign', 'companies.create', 'companies.read', 'companies.update', 'companies.delete', 'companies.export', 'companies.duplicate', 'branches.create', 'branches.read', 'branches.update', 'branches.delete', 'branches.export', 'branches.duplicate', 'clients.create', 'clients.read', 'clients.update', 'clients.delete', 'clients.export', 'clients.duplicate', 'vendors.create', 'vendors.read', 'vendors.update', 'vendors.delete', 'vendors.export', 'vendors.duplicate', 'products.create', 'products.read', 'products.update', 'products.delete', 'products.export', 'products.duplicate', 'invoices.create', 'invoices.read', 'invoices.update', 'invoices.delete', 'invoices.export', 'invoices.duplicate', 'expenses.create', 'expenses.read', 'expenses.update', 'expenses.delete', 'expenses.export', 'expenses.duplicate', 'settings.read', 'settings.update', 'users.duplicate', 'roles.duplicate', 'jobs.read', 'jobs.create', 'jobs.delete', 'jobs.update', 'jobs.duplicate', 'jobs.export', 'job_listings.read', 'job_listings.create', 'job_listings.delete', 'job_listings.update', 'job_listings.duplicate', 'job_listings.export', 'departments.read', 'departments.create', 'departments.delete', 'departments.update', 'departments.duplicate', 'departments.export', 'salaries.read', 'salaries.create', 'salaries.delete', 'salaries.update', 'salaries.duplicate', 'salaries.export', 'offices.read', 'offices.create', 'offices.delete', 'offices.update', 'offices.duplicate', 'offices.export', 'warehouses.read', 'warehouses.create', 'warehouses.delete', 'warehouses.update', 'warehouses.duplicate', 'warehouses.export', 'employees.read', 'employees.create', 'employees.delete', 'employees.update', 'employees.duplicate', 'employees.export', 'employee_requests.read', 'employee_requests.create', 'employee_requests.delete', 'employee_requests.update', 'employee_requests.duplicate', 'employee_requests.export', 'quotes.read', 'quotes.create', 'quotes.delete', 'quotes.update', 'quotes.duplicate', 'quotes.export', 'activity_logs.read', 'activity_logs.delete', 'activity_logs.export', 'domains.read', 'domains.create', 'domains.delete', 'domains.update', 'domains.export', 'servers.read', 'servers.create', 'servers.delete', 'servers.update', 'servers.export', 'servers.duplicate', 'purchases.read', 'purchases.create', 'purchases.delete', 'purchases.update', 'purchases.export', 'purchases.duplicate', 'websites.read', 'websites.create', 'websites.delete', 'websites.update', 'websites.export', 'websites.duplicate', 'online_stores.read', 'online_stores.create', 'online_stores.delete', 'online_stores.update', 'online_stores.export', 'online_stores.duplicate', 'cars.read', 'cars.create', 'cars.delete', 'cars.update', 'cars.export', 'cars.duplicate', 'trucks.read', 'trucks.create', 'trucks.delete', 'trucks.update', 'trucks.export', 'trucks.duplicate'])
...
export const employee_status = pgEnum("employee_status", ['active', 'inactive', 'terminated', 'on_leave', 'resigned'])
export const payment_cycle = pgEnum("payment_cycle", ['monthly', 'annual'])


export const clients = pgTable("clients", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	created_at: timestamp({ withTimezone: true, mode: 'string' }).default(sql`timezone('utc'::text, now())`),
	name: text().notNull(),
	email: text(),
	phone: text().notNull(),
	short_address: text(),
	...
	index("idx_clients_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
	foreignKey({
...
export const branches = pgTable("branches", {
	...
	index("idx_branches_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const departments = pgTable("departments", {
	...
	description: text(),
	user_id: uuid().notNull(),
	updated_at: timestamp({ withTimezone: true, mode: 'string' }).default(sql`timezone('utc'::text, now())`),
	status: common_status().default('active'),
	enterprise_id: uuid().notNull(),
	notes: jsonb(),
}, (table) => [
	...
	index("idx_departments_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
]);
...
export const companies = pgTable("companies", {
	...
	index("idx_companies_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
]);
...
export const memberships = pgTable("memberships", {
	...
	pgPolicy("Access if same user", { as: "permissive", for: "select", to: ["public"], using: sql`(profile_id = auth.uid())` }),
]);
...
export const enterprises = pgTable("enterprises", {
	...
	pgPolicy("Access if member", { as: "permissive", for: "select", to: ["public"], using: sql`(EXISTS ( SELECT 1
   FROM memberships
...
export const invoice_items = pgTable("invoice_items", {
	...
	index("invoice_items_invoice_id_idx").using("btree", table.invoice_id.asc().nullsLast().op("uuid_ops")),
	foreignKey({
...
export const mfa_amr_claimsInAuth = auth.table("mfa_amr_claims", {
	...
	unique("mfa_amr_claims_session_id_authentication_method_pkey").on(table.session_id, table.authentication_method),
]);
...
export const sessionsInAuth = auth.table("sessions", {
	...
	index("user_id_created_at_idx").using("btree", table.user_id.asc().nullsLast().op("uuid_ops"), table.created_at.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.user_id],
			foreignColumns: [usersInAuth.id],
...
export const user_enterprise_roles = pgTable("user_enterprise_roles", {
	id: uuid().default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	user_id: uuid().notNull(),
	enterprise_id: uuid().notNull(),
	...
	unique("user_enterprise_roles_user_id_enterprise_id_key").on(table.user_id, table.enterprise_id),
	pgPolicy("Enable read access for users", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(auth.uid() = user_id)` }),
...
export const jobs = pgTable("jobs", {
	...
	index("idx_jobs_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const department_locations = pgTable("department_locations", {
	...
	unique("unique_department_location").on(table.department_id, table.location_type, table.location_id),
...
export const documents = pgTable("documents", {
	...
	index("documents_entity_id_idx").using("btree", table.entity_id.asc().nullsLast().op("uuid_ops")),
	index("documents_entity_type_idx").using("btree", table.entity_type.asc().nullsLast().op("text_ops")),
...
export const job_listing_jobs = pgTable("job_listing_jobs", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	...
	index("job_listing_jobs_job_id_idx").using("btree", table.job_id.asc().nullsLast().op("uuid_ops")),
	index("job_listing_jobs_job_listing_id_idx").using("btree", table.job_listing_id.asc().nullsLast().op("uuid_ops")),
	foreignKey({
...
export const employee_requests = pgTable("employee_requests", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	...
	index("employee_requests_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")),
	index("employee_requests_employee_id_idx").using("btree", table.employee_id.asc().nullsLast().op("uuid_ops")),
	...
	index("employee_requests_user_id_idx").using("btree", table.user_id.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.employee_id],
			foreignColumns: [employees.id],
			name: "employee_requests_employee_id_employees_id_fk"
		}),
	pgPolicy("Allow enterprise members to update requests", { as: "permissive", for: "update", to: ["public"], using: sql`is_member_of_enterprise(enterprise_id)`, withCheck: sql`is_member_of_enterprise(enterprise_id)`  }),
	pgPolicy("Allow members to view own enterprise requests", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("Allow members to insert for own enterprise", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Allow creator to delete own requests", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const profiles = pgTable("profiles", {
	...
	index("profiles_enterprise_id_idx").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const quote_items = pgTable("quote_items", {
	...
	index("quote_items_quote_id_idx").using("btree", table.quote_id.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.product_id],
			foreignColumns: [products.id],
...
export const roles = pgTable("roles", {
	...
	pgPolicy("Users can view roles in their enterprise", { as: "permissive", for: "select", to: ["public"], using: sql`((enterprise_id IN ( SELECT user_enterprises.enterprise_id
   FROM user_enterprises
  WHERE (user_enterprises.user_id = auth.uid()))) OR (is_system = true))` }),
	pgPolicy("Users can create roles in their enterprise", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Users can update roles in their enterprise", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can delete roles in their enterprise", { as: "permissive", for: "delete", to: ["public"] }),
]);
...
export const permissions = pgTable("permissions", {
	...
	pgPolicy("Users can view permissions in their enterprise", { as: "permissive", for: "select", to: ["public"], using: sql`(role_id IN ( SELECT roles.id
   FROM roles
  WHERE ((roles.enterprise_id IN ( SELECT user_enterprises.enterprise_id
	...
	pgPolicy("Users can manage permissions in their enterprise", { as: "permissive", for: "all", to: ["public"] }),
]);
...
export const employees = pgTable("employees", {
	...
	index("idx_employees_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.job_id],
			foreignColumns: [jobs.id],
...
export const cars = pgTable("cars", {
	...
	index("cars_enterprise_id_idx").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const domains = pgTable("domains", {
	...
	index("domains_domain_name_idx").using("btree", table.domain_name.asc().nullsLast().op("text_ops")),
	index("domains_enterprise_id_idx").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
END`),
	...
	index("idx_invoices_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
	index("invoices_client_id_idx").using("btree", table.client_id.asc().nullsLast().op("uuid_ops")),
	index("invoices_invoice_number_idx").using("btree", table.invoice_number.asc().nullsLast().op("text_ops")),
	...
	check("invoices_status_check", sql`status = ANY (ARRAY['draft'::text, 'sent'::text, 'paid'::text, 'partially_paid'::text, 'overdue'::text, 'void'::text])`),
]);
...
export const offices = pgTable("offices", {
	...
	index("idx_offices_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const online_stores = pgTable("online_stores", {
	id: uuid().default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	domain_name: text().notNull(),
	created_at: timestamp({ withTimezone: true, mode: 'string' }).default(sql`timezone('utc'::text, now())`).notNull(),
	updated_at: timestamp({ withTimezone: true, mode: 'string' }).default(sql`timezone('utc'::text, now())`).notNull(),
	...
	index("online_stores_domain_name_idx").using("btree", table.domain_name.asc().nullsLast().op("text_ops")),
	index("online_stores_enterprise_id_idx").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
	index("online_stores_user_id_idx").using("btree", table.user_id.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.enterprise_id],
			foreignColumns: [enterprises.id],
	...
	unique("online_stores_enterprise_id_domain_name_unique").on(table.domain_name, table.enterprise_id),
]);
...
export const products = pgTable("products", {
	...
	index("idx_products_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const salaries = pgTable("salaries", {
	...
	index("salaries_employee_id_idx").using("btree", table.employee_id.asc().nullsLast().op("uuid_ops")),
...
export const servers = pgTable("servers", {
	...
	index("servers_enterprise_id_idx").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
	index("servers_ip_address_idx").using("btree", table.ip_address.asc().nullsLast().op("inet_ops")),
...
export const trucks = pgTable("trucks", {
	...
	index("trucks_enterprise_id_idx").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const vendors = pgTable("vendors", {
	...
	index("idx_vendors_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const warehouses = pgTable("warehouses", {
	...
	index("idx_warehouses_enterprise_id").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const websites = pgTable("websites", {
	...
	index("websites_domain_name_idx").using("btree", table.domain_name.asc().nullsLast().op("text_ops")),
	index("websites_enterprise_id_idx").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
...
export const subscription_reactivations = pgTable("subscription_reactivations", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	user_id: uuid().notNull(),
	...
	pgPolicy("Users can view their own reactivation history", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
	pgPolicy("Users can add their own reactivation records", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("No one can update reactivation history", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("No one can delete reactivation history", { as: "permissive", for: "delete", to: ["public"] }),
	pgPolicy("Service role can manage all reactivation records", { as: "permissive", for: "all", to: ["public"] }),
]);
...
export const user_roles = pgTable("user_roles", {
	...
	index("user_roles_enterprise_id_idx").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
	...
	primaryKey({ columns: [table.user_id, table.role_id, table.enterprise_id], name: "user_roles_pkey"}),
]);
...
export const activity_logs = pgTable("activity_logs", {
	...
	index("activity_logs_branch_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'BRANCH'::activity_target_type)`),
	index("activity_logs_branch_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'BRANCH'::activity_target_type)`),
	index("activity_logs_cars_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'CAR'::activity_target_type)`),
	index("activity_logs_cars_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'CAR'::activity_target_type)`),
	index("activity_logs_client_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'CLIENT'::activity_target_type)`),
	index("activity_logs_client_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'CLIENT'::activity_target_type)`),
	index("activity_logs_domain_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'DOMAIN'::activity_target_type)`),
	index("activity_logs_domain_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'DOMAIN'::activity_target_type)`),
	index("activity_logs_employee_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'EMPLOYEE'::activity_target_type)`),
	index("activity_logs_employee_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'EMPLOYEE'::activity_target_type)`),
	index("activity_logs_employee_request_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'EMPLOYEE_REQUEST'::activity_target_type)`),
	index("activity_logs_employee_request_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'EMPLOYEE_REQUEST'::activity_target_type)`),
	index("activity_logs_enterprise_idx").using("btree", table.enterprise_id.asc().nullsLast().op("uuid_ops")),
	index("activity_logs_expense_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'EXPENSE'::activity_target_type)`),
	index("activity_logs_expense_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'EXPENSE'::activity_target_type)`),
	index("activity_logs_invoice_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'INVOICE'::activity_target_type)`),
	index("activity_logs_invoice_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'INVOICE'::activity_target_type)`),
	index("activity_logs_job_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'JOB'::activity_target_type)`),
	index("activity_logs_job_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'JOB'::activity_target_type)`),
	index("activity_logs_job_listing_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'JOB_LISTING'::activity_target_type)`),
	index("activity_logs_job_listing_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'JOB_LISTING'::activity_target_type)`),
	index("activity_logs_office_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'OFFICE'::activity_target_type)`),
	index("activity_logs_office_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'OFFICE'::activity_target_type)`),
	index("activity_logs_online_stores_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'ONLINE_STORE'::activity_target_type)`),
	index("activity_logs_online_stores_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'ONLINE_STORE'::activity_target_type)`),
	index("activity_logs_product_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'PRODUCT'::activity_target_type)`),
	index("activity_logs_product_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'PRODUCT'::activity_target_type)`),
	index("activity_logs_purchases_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'PURCHASE'::activity_target_type)`),
	index("activity_logs_purchases_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'PURCHASE'::activity_target_type)`),
	index("activity_logs_server_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'SERVER'::activity_target_type)`),
	index("activity_logs_server_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'SERVER'::activity_target_type)`),
	index("activity_logs_target_idx").using("btree", table.target_type.asc().nullsLast().op("enum_ops"), table.target_id.asc().nullsLast().op("enum_ops")),
	index("activity_logs_user_idx").using("btree", table.user_id.asc().nullsLast().op("uuid_ops")),
	index("activity_logs_warehouse_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'WAREHOUSE'::activity_target_type)`),
	index("activity_logs_warehouse_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'WAREHOUSE'::activity_target_type)`),
	index("activity_logs_website_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'WEBSITE'::activity_target_type)`),
	index("activity_logs_website_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'WEBSITE'::activity_target_type)`),
	index("activity_logs_websites_action_idx").using("btree", table.action_type.asc().nullsLast().op("enum_ops")).where(sql`(target_type = 'WEBSITE'::activity_target_type)`),
	index("activity_logs_websites_created_at_idx").using("btree", table.created_at.asc().nullsLast().op("timestamptz_ops")).where(sql`(target_type = 'WEBSITE'::activity_target_type)`),
	foreignKey({
			columns: [table.enterprise_id],
			foreignColumns: [enterprises.id],
...
export const user_permissions_view = pgView("user_permissions_view", {	user_id: uuid(),
	permission_id: uuid(),
	...
	enterprise_id: uuid(),
	is_system: boolean(),
}).as(sql`SELECT DISTINCT m.profile_id AS user_id, p.id AS permission_id, p.permission::text AS permission_name, m.role_id, r.name AS role_name, m.enterprise_id, r.is_system FROM memberships m JOIN roles r ON m.role_id = r.id JOIN permissions p ON r.id = p.role_id`);

export const user_enterprises = pgView("user_enterprises", {	user_id: uuid(),
	enterprise_id: uuid(),
