...
CREATE OR REPLACE FUNCTION public.module_log_cars()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE TRIGGER trigger_cars_activity_log    
AFTER INSERT OR UPDATE OR DELETE ON public.cars
FOR EACH ROW EXECUTE FUNCTION public.module_log_cars();


-- Drop existing function if needed
DROP FUNCTION IF EXISTS get_module_analytics_cars;

-- Create new function with full implementation
CREATE OR REPLACE FUNCTION get_module_analytics_cars(
...
BEGIN
  ...
  SELECT 
    ts.period_start,
    COUNT(al.*) FILTER (WHERE al.action_type = 'CREATED')::BIGINT AS cars_added,
    COUNT(al.*) FILTER (WHERE al.action_type = 'DELETED')::BIGINT AS cars_removed,
    COUNT(al.*) FILTER (WHERE al.action_type = 'UPDATED')::BIGINT AS cars_updated
  FROM time_series ts
  LEFT JOIN activity_logs al ON 
    date_trunc(time_interval, al.created_at AT TIME ZONE 'UTC') = ts.period_start
    AND al.target_type = 'CAR'
...
EXCEPTION
  WHEN others THEN
    RAISE WARNING 'Error in get_module_analytics_cars: %', SQLERRM;
    RETURN;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;

-- Set permissions
REVOKE ALL ON FUNCTION get_module_analytics_cars FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_module_analytics_cars TO authenticated;

-- Create indexes
CREATE INDEX IF NOT EXISTS activity_logs_cars_created_at_idx
ON activity_logs (created_at)
WHERE target_type = 'CAR';

CREATE INDEX IF NOT EXISTS activity_logs_cars_action_idx
