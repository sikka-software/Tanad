...
import { companies, clients, employees, branches, enterprises, memberships, profiles, roles, products, invoice_items, usersInAuth, one_time_tokensInAuth, sessionsInAuth, refresh_tokensInAuth, identitiesInAuth, mfa_amr_claimsInAuth, mfa_factorsInAuth, mfa_challengesInAuth, sso_providersInAuth, saml_providersInAuth, flow_stateInAuth, saml_relay_statesInAuth, sso_domainsInAuth, user_enterprise_roles, departments, department_locations, jobs, job_listing_jobs, job_listings, employee_requests, quote_items, quotes, permissions, expenses, cars, domains, invoices, offices, online_stores, purchases, salaries, servers, trucks, warehouses, websites, subscription_reactivations, user_roles, activity_logs } from "./schema";
...
export const subscription_reactivationsRelations = relations(subscription_reactivations, ({one}) => ({
	usersInAuth: one(usersInAuth, {
