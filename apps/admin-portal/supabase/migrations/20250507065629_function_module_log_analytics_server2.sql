...
CREATE OR REPLACE FUNCTION public.module_log_server()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE TRIGGER trigger_servers_activity_log
AFTER INSERT OR UPDATE OR DELETE ON public.servers
FOR EACH ROW EXECUTE FUNCTION public.module_log_server();


-- Drop existing function if needed
DROP FUNCTION IF EXISTS get_module_analytics_server;

-- Create new function with full implementation
CREATE OR REPLACE FUNCTION get_module_analytics_server(
...
RETURNS TABLE (
  ...
  servers_added BIGINT,
  servers_removed BIGINT,
  servers_updated BIGINT
) AS $$
DECLARE
...
BEGIN
  ...
  SELECT 
    ts.period_start,
    COUNT(al.*) FILTER (WHERE al.action_type = 'CREATED')::BIGINT AS servers_added,
    COUNT(al.*) FILTER (WHERE al.action_type = 'DELETED')::BIGINT AS servers_removed,
    COUNT(al.*) FILTER (WHERE al.action_type = 'UPDATED')::BIGINT AS servers_updated
  FROM time_series ts
  LEFT JOIN activity_logs al ON 
    date_trunc(time_interval, al.created_at AT TIME ZONE 'UTC') = ts.period_start
    AND al.target_type = 'SERVER'
...
EXCEPTION
  WHEN others THEN
    RAISE WARNING 'Error in get_module_analytics_server: %', SQLERRM;
    RETURN;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;

-- Set permissions
REVOKE ALL ON FUNCTION get_module_analytics_server FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_module_analytics_server TO authenticated;

-- Create indexes
CREATE INDEX IF NOT EXISTS activity_logs_server_created_at_idx
ON activity_logs (created_at)
WHERE target_type = 'SERVER';

CREATE INDEX IF NOT EXISTS activity_logs_server_action_idx
