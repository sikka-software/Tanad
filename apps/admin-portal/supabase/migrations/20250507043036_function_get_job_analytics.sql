...
DROP FUNCTION IF EXISTS get_module_analytics_job;

-- Create new function with full implementation
CREATE OR REPLACE FUNCTION get_module_analytics_job(
...
BEGIN
  ...
  SELECT 
    ts.period_start,
    COUNT(al.*) FILTER (WHERE al.action_type = 'CREATED')::BIGINT AS jobs_added,
    COUNT(al.*) FILTER (WHERE al.action_type = 'DELETED')::BIGINT AS jobs_removed,
    COUNT(al.*) FILTER (WHERE al.action_type = 'UPDATED')::BIGINT AS jobs_updated
  FROM time_series ts
  LEFT JOIN activity_logs al ON 
    date_trunc(time_interval, al.created_at AT TIME ZONE 'UTC') = ts.period_start
    AND al.target_type = 'JOB'
...
EXCEPTION
  WHEN others THEN
    RAISE WARNING 'Error in get_module_analytics_job: %', SQLERRM;
    RETURN;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;

-- Set permissions
REVOKE ALL ON FUNCTION get_module_analytics_job FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_module_analytics_job TO authenticated;

-- Create indexes
CREATE INDEX IF NOT EXISTS activity_logs_job_created_at_idx
ON activity_logs (created_at)
WHERE target_type = 'JOB';

CREATE INDEX IF NOT EXISTS activity_logs_job_action_idx
