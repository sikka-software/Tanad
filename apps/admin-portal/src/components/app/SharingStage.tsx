  ...
  return (
    <div className="flex flex-col gap-2" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <ShareViaPlatformButton
        ...
        handleClick={() => window.open(snapchatLink, "SnapchatShare", "width=600,height=600")}
      />
      <ShareViaPlatformButton
...
const ShareViaPlatformButton = (props: { handleClick?: () => void; title?: string }) => {
  const lang = useLocale();
  return (
    <Button
      ...
      variant="ghost"
      autoFocus={false}
      className="flex h-12 min-h-10 flex-row justify-between text-start"
    >
      <span>{props.title}</span> <ChevronRight className={lang === "ar" ? "rotate-180" : ""} />
    </Button>
  );
};
