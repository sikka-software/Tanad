...
const buildClipPath = (top: number, right: number, bottom: number, left: number) => {
...
export const SelectionRect = React.memo<SelectionContextType>(
  ({
    ...
    const selectionRect = selection && {
      width:
        columnWidths.slice(selection.min.col + 1, selection.max.col + 2).reduce((a, b) => a + b) +
    ...
    const expandRowsIndicator = maxSelection &&
      expandSelection !== null && {
        ...
        transform: `translate(-${
          maxSelection.col < columnWidths.length - (hasStickyRightColumn ? 3 : 2) ? 50 : 100
    ...
    const expandRowsRect = minSelection &&
      ...
      expandSelection !== null && {
        width:
          columnWidths.slice(minSelection.col + 1, maxSelection.col + 2).reduce((a, b) => a + b) +
          extraPixelH(maxSelection.col),
        height:
    ...
    return (
      <>
        <div
          ...
          style={{
            height:
              rowHeight(dataLength - 1).top + rowHeight(dataLength - 1).height + headerRowHeight,
        ...
        {expandRowsRect && <div className={cx("dsg-expand-rows-rect")} style={expandRowsRect} />}
        {expandRowsIndicator && (
          <div
            className={cx(
