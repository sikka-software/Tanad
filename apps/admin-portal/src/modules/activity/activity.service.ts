...
export class ActivityService {
  ...
  static async exportList(
    ...
    if (enterpriseError) {
      console.error("ActivityService.exportList: Error fetching enterprise ID:", enterpriseError);
      throw new Error("Failed to retrieve enterprise association.");
    }
    if (!membership?.enterprise_id) {
      console.warn("ActivityService.exportList: User is not associated with an enterprise.");
      return [];
    }
    const enterprise_id = membership.enterprise_id;
    ...
    if (error) {
      console.error("ActivityService.exportList error calling RPC:", error);
      throw error;
    }
