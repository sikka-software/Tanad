...
import { VehicleUpdateData, VehicleCreateData } from "@/vehicle/vehicle.type";

export const createVehicleSchema = (t: (key: string) => string) => {
  const VehicleSelectSchema = createInsertSchema(vehicles, {
    // name: z
  ...
  return VehicleSelectSchema;
};

export type VehicleFormValues = z.input<ReturnType<typeof createVehicleSchema>>;
...
export function VehicleForm({
  ...
  const setIsLoadingSave = useVehicleStore((state) => state.setIsLoading);

  const form = useForm<VehicleFormValues>({
    resolver: zodResolver(createVehicleSchema(t)),
    defaultValues: {
      ...defaultValues,
        ...
        <div className="form-container">
          ...
          <div className="form-fields-cols-2">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t("Vehicles.form.code.label")}</FormLabel>
                  <FormControl>
                    <Input
                      placeholder={t("Vehicles.form.code.placeholder")}
                      disabled={isLoading}
                      {...field}
                      value={field.value ?? ""}
                    />
            ...
            {/* <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t("Vehicles.form.name.label")}</FormLabel>
                  <FormControl>
                    <Input
                      placeholder={t("Vehicles.form.name.placeholder")}
                      {...field}
                      disabled={isLoading}
                    />
                  </FormControl>
